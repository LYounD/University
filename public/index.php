...
try {
  ...
  $sql = "INSERT INTO table_text (add_txt) VALUES (:add_text)";
  $prepare = $db->prepare($sql);
  $prepare->bindValue(':add_text', $add_text, PDO::PARAM_STR);  
  $prepare->execute();
  $id = $db->lastInsertId();
...
?>
    ...
    <form name="form1">
       <input type="text" name="textclock1" size="40" style=" text-align:right; border : 0; width:100%;">
    </form>
    <hr size="1" width="90%" align="center" color=#6799FF>
    <div id="Memo_input" style='text-align: left; padding: 0px 0px 0px 31px'>
      <input style ='width: 80%' type = "text" id = "text_title" placeholder="제목">
      <br>
      <form method='get' action="<? echo  $_Get['txt_input']; ?>">
      <textarea name= 'txt_input' id='txt_input' style="width: 80%" rows=10 placeholder="본문"></textarea>
      </form>
